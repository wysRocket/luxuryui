...
import { ArrowRight, Coins, Euro, PackageCheck, PoundSterling, Wallet } from 'lucide-react';
...
const CreditsPage: React.FC = () => {
  ...
  const handleTopUp = async () => {
    ...
    try {
      ...
      setErrorMessage(error instanceof Error ? error.message : 'Could not complete the credit top-up.');
    }
  };
      ...
      <section className="grid grid-cols-1 xl:grid-cols-[1.25fr_0.75fr] gap-8 mb-10">
        <article className="rounded-[32px] border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 p-6 md:p-8">
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-8">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.2em] text-gray-400 dark:text-gray-500 mb-3">Credit Top-Up</p>
              <h2 className="text-3xl md:text-4xl font-black tracking-tight text-gray-900 dark:text-white">Choose your credits</h2>
            </div>
            <div className="rounded-[24px] bg-gray-50 dark:bg-gray-950/50 px-5 py-4 border border-gray-100 dark:border-gray-800">
              <p className="text-xs font-black uppercase tracking-[0.18em] text-gray-400 dark:text-gray-500 mb-2">Selected</p>
              <div className="flex items-baseline gap-2">
                <span className="text-4xl font-black tracking-tight text-gray-900 dark:text-white">{quote.credits}</span>
            ...
            <div className="flex flex-col xl:flex-row xl:items-center gap-4">
              ...
              <div className="flex-1 px-1">
                <input
                  ...
                  value={quote.credits}
                  onChange={(event) => setCredits(clampCredits(Number(event.target.value)))}
          ...
          <div className="mb-6 rounded-[24px] border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-950/40 px-5 py-5">
            <p className="text-xs font-black uppercase tracking-[0.16em] text-gray-400 dark:text-gray-500 mb-2">Current wallet</p>
            <p className="text-2xl font-black text-gray-900 dark:text-white">{availableCredits} credits</p>
          </div>
        ...
        <aside className="space-y-6">
          <article className="rounded-[32px] border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 p-6">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-gray-400 dark:text-gray-500 mb-4">How It Works</p>
            <div className="space-y-4 text-sm leading-relaxed text-gray-600 dark:text-gray-400">
              ...
              <p>3. Spend credits on approved Figma kits with editable files and commercial packaging.</p>
            </div>
          </article>
          ...
          <article className="rounded-[32px] border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 p-6">
            <div className="flex items-center justify-between gap-3 mb-4">
              <div>
                <p className="text-xs font-black uppercase tracking-[0.2em] text-gray-400 dark:text-gray-500 mb-2">Unlockable Now</p>
                <h3 className="text-2xl font-black tracking-tight text-gray-900 dark:text-white">Kits within this top-up</h3>
              </div>
            ...
            {affordableKits.length > 0 ? (
              <div className="space-y-3">
                {affordableKits.map((kit) => (
                  ...
                  >
                    <p className="text-sm font-black text-gray-900 dark:text-white mb-1">{kit.title}</p>
                    ...
                    <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                      {availableCredits + quote.credits >= kit.creditCost ? 'Within your current balance + selected top-up' : 'Requires a larger top-up'}
                    </p>
                  </Link>
                ))}
              </div>
