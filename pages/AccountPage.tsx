        ...
        {backendMode === "firebase" && (
          <div className="mb-6 flex flex-wrap items-center gap-2">
            <div className="inline-flex items-center rounded-full border border-emerald-200 bg-emerald-50 px-4 py-2 text-xs font-black uppercase tracking-[0.16em] text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-950/30 dark:text-emerald-200">
              Firebase account with synced wallet
            </div>
            <div className="inline-flex items-center rounded-full border border-gray-200 bg-gray-50 px-4 py-2 text-xs font-black uppercase tracking-[0.14em] text-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-200">
              {providerLabel}
            </div>
          </div>
        )}
      ...
      <section className="grid grid-cols-1 xl:grid-cols-[1fr_0.9fr] gap-8">
        <div className="space-y-8">
          <article className="rounded-[28px] border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 p-6">
            ...
            {unlocks.length > 0 ? (
              <div className="space-y-3">
                {unlocks.map((unlock) => {
                  const kit = FIGMA_KIT_PRODUCTS.find(
              ...
              <div className="rounded-2xl border border-dashed border-gray-200 dark:border-gray-700 px-4 py-5 text-sm text-gray-500 dark:text-gray-400">
                No unlocked kits yet. Top up credits and buy your first Figma
                kit.
              </div>
            )}
          </article>
          ...
          <article className="rounded-[28px] border border-gray-100 dark:border-gray-800 bg-white dark:bg-gray-900 p-6">
            ...
            <h2 className="text-2xl font-black tracking-tight text-gray-900 dark:text-white mb-5">
              Wallet history
            </h2>
            ...
            {transactions.length > 0 ? (
              <div className="space-y-3">
                {transactions.slice(0, 8).map((transaction) => (
                  <div
                    key={transaction.id}
                    className="flex items-center justify-between gap-3 rounded-2xl border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-950/40 px-4 py-4"
                  >
                    <div>
                    ...
                    <div
                      className={`text-sm font-black ${transaction.creditsDelta >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-gray-900 dark:text-white"}`}
                    >
            ...
            {topUps.length > 0 ? (
              <div className="space-y-3">
                {topUps.slice(0, 6).map((topUp) => (
                  <div
                    key={topUp.id}
                    className="rounded-2xl border border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-950/40 px-4 py-4"
                  >
                    <p className="text-sm font-black text-gray-900 dark:text-white">
