...
async function run() {
    ...
    try {
        ...
        for (let attempt = 1; attempt <= MAX_TOOL_CALL_ATTEMPTS; attempt++) {
            try {
                payload = await callDeployTool();
                break;
            } catch (error) {
                if (!isRetryableDeployError(error) || attempt === MAX_TOOL_CALL_ATTEMPTS) {
                    throw error;
                }

                console.warn(
                    `Hostinger deploy returned a transient 500 (attempt ${attempt}/${MAX_TOOL_CALL_ATTEMPTS}). Retrying in ${TOOL_CALL_RETRY_DELAY_MS / 1000}s...`
                );
                await sleep(TOOL_CALL_RETRY_DELAY_MS);
            }
        }
